...
<body>
    <?php 
    include "koneksi.php";
    $qry_get_paket=mysqli_query($conn,"select * from merek where id_merek = '".$_GET['id_merek']."'");
    ...
    <form action="proses_ubah_paket.php" method="post">
        ...
        <select name="jenis" class="form-control">
            <option></option>
            <?php foreach ($arr_jenis as $key_jenis_merek => $val_jenis_merek):
                if($key_jenis_merek==$dt_merek['jenis merek']){
                    $selek="selected";
                } else {
                    $selek="";
                }
        ...
        Harga :
        <input type="number" name="harga" value=   "<?=$dt_merek['harga']?>" class="form-control">
